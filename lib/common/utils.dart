  ...
  try {
    ...
    for (String key in authKeys) {
      try {
        ...
        print('Warning: Could not remove key $key: $e');
      }
    }
    ...
    try {
      await prefs.clear();
      print('Info: Successfully cleared all SharedPreferences');
    } catch (e) {
      print('Warning: Could not clear all preferences: $e');
    }
    
    return true;
  } catch (e) {
    print('Warning: Could not recover from keychain error: $e');
    return false;
  }
}
...
Future<T?> safeLocalStorageAccess<T>(
  ...
  try {
    ...
    print('Warning: Local storage access failed: $e');
    return defaultValue;
  }
}
